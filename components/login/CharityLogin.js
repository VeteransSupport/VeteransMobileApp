...
export default class CharityLogin extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      authenticated: false,
  ...
  redirect = () => {
    ...
    } else if (this.state.userTypeId === '1') {
  ...
  handleLoginClick = async () => {
    let url = 'http://urbackup.atwebpages.com/api/authenticate';
    let formData = new FormData()
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.title}>{'Charity\nLog-in'}</Text>
